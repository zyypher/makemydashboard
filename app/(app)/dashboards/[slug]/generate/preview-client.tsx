...
function friendlyMode(mode: Mode) {
  switch (mode) {
    case "PRIMARY":
      return "Main record";
    case "FIXED":
...
type PageKey = "overview" | "main" | `managed:${string}`;
...
export default function GeneratePreviewClient(props: {
  ...
  const { dashboardName, model, preview } = props;

  const rows = React.useMemo(() => preview.rows ?? [], [preview.rows]);

  const primaryKey = model.primaryFieldKey;
  const mainEntityName = dashboardName; // per your decision
  ...
  const fixedFields = model.fields
    ...
    .map((f) => ({ key: f.key, options: f.fixedOptions ?? uniq(rows.map((r) => r[f.key])) }));
  ...
  const visibleTableFields = React.useMemo(() => {
    // For list view: show Primary + a few other non-ignored fields
  ...
  const kpis = React.useMemo(() => {
    ...
    // pick 2 managed fields to show as “counts”
    const managedCounts = managedFields.slice(0, 3).map((k) => ({
      key: k,
      value: countUnique(rows, k),
    }));

    // pick 1 fixed field to show “options”
  ...
  const sidebarItems: Array<{ key: PageKey; label: string; icon: React.ReactNode }> =
    React.useMemo(() => {
      ...
      items.push({ key: "settings" as any, label: "Settings (later)", icon: <Settings className="h-4 w-4" /> });

      return items;
    }, [managedFields, mainEntityName]);

  const managedKey = typeof active === "string" && active.startsWith("managed:")
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-neutral-200 bg-neutral-50 p-4 text-sm text-neutral-700">
        <span className="font-semibold">Preview only:</span> You’ll be able to change logo,
        theme, and layouts later. This shows what we’ll generate from your sheet.
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-neutral-200 bg-white">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              ...
              <div className="text-xs text-neutral-500">
                Logo + theme can be updated later
              </div>
            </div>
          </div>
        ...
        <div className="grid min-h-[520px] grid-cols-1 md:grid-cols-12">
          ...
          <div className="md:col-span-3 border-b md:border-b-0 md:border-r border-neutral-200 bg-white">
            ...
            {/* “Generated from your sheet” info */}
          ...
          <div className="md:col-span-9 p-4">
            {active === "overview" ? (
              ...
              <div className="text-sm text-neutral-600">
                Settings preview will be added later.
              </div>
            )}

            {/* Bottom: “Create form preview” */}
            <div className="mt-6 rounded-2xl border border-neutral-200 bg-white">
              <div className="border-b border-neutral-200 p-4">
                ...
                <div className="mt-1 text-xs text-neutral-600">
                  This is how the form will look. Fixed choices become dropdowns.
                </div>
              </div>
            ...
            <div className="mt-4 text-xs text-neutral-500">
              Next we’ll add a real “Create pages” action that generates these screens and links them to your sheet.
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <div className="text-sm text-neutral-600">
          Quick summary based on your sheet sample.
        </div>
      </div>
