...
import { Bell, Moon, Search, Settings, Star, Languages } from "lucide-react";
    ...
    <header className="sticky top-0 z-30 border-b border-neutral-200 bg-white/80 backdrop-blur-xl">
      <div className="mx-auto flex h-16 w-full max-w-[1400px] items-center gap-3 px-4 md:px-6">
        ...
        <div className="relative" ref={wrapRef}>
          <button
            onClick={() => setOpen((v) => !v)}
            className="ml-1 grid h-10 w-10 place-items-center rounded-full ring-2 ring-neutral-200 hover:ring-neutral-300"
            aria-label="Profile"
          >
          ...
          {open ? (
            <div className="absolute right-0 mt-3 w-72 overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-[0_18px_60px_-22px_rgba(0,0,0,0.35)]">
              <div className="px-4 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="leading-tight">
                    <div className="text-sm font-semibold text-neutral-900">Vishnu</div>
              ...
              <div className="p-2">
                <MenuItem icon={<Settings className="h-4.5 w-4.5" />} label="Settings" />
                <button
                  onClick={() => signOut({ callbackUrl: "/login" })}
                  className="mt-2 flex w-full items-center justify-center rounded-xl bg-rose-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-rose-600"
                >
                  Logout
                </button>
              </div>
...
function IconButton({ children }: { children: React.ReactNode }) {
  return (
    <button className="grid h-10 w-10 place-items-center rounded-xl text-neutral-500 hover:bg-neutral-100 hover:text-neutral-800">
      {children}
    </button>
  );
}
...
function MenuItem({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <button className="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm text-neutral-700 hover:bg-neutral-100">
